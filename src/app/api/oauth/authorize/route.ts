 ...
 *     If found AND credentials still exist → silent re-auth: skip Google login,
 *     issue an auth code directly and redirect back to the MCP client.
 *  3. Otherwise → save session and redirect user to /connect for Google sign-in.
 */
...
export async function GET(req: NextRequest) {
  ...
  if (cookieUserId) {
    ...
    if (creds) {
      // Credentials still valid → skip Google login, issue code immediately
