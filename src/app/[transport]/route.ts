 ...
 * to request.auth.extra, then the inner handler loads Google credentials
 * and creates the per-request GTMClient.
 *
...
import { GTMClient } from "@/lib/google-tagmanager";
...
async function mcpHandler(request: Request): Promise<Response> {
  ...
  const client = new GTMClient(userId);
